    ...
    <div class="m-body">
      <div v-if="payments.length && !loading">
        <ul class="deposit-main">
          ...
          <li class="quick-select">
            <div class="board">
              <div v-if="moneyList.length > 0" class="chips">
                <div
                  class="chip"
              ...
              <div class="list-cells">
                ...
                <div class="cell" v-if="payment && (payment.id === 12 || payment.id === 13)">
                  <template v-if="qrCodeUrl">
                    <div
                      class="cell-head"
                    >{{ payment.id === 12 ? $t('微信') : $t('支付宝') }}{{ $t('昵称') }}</div>
                    <div class="cell-body">
                      <input
                        type="text"
                        v-if="payment.id === 12"
                        v-model="form.alipayName"
                        :placeholder="$t('请输入微信昵称，转账备注会员账号')"
                      />
                      <input
                        type="text"
                        v-else
                        v-model="form.alipayName"
                        :placeholder="$t('请输入支付宝昵称，转账备注会员账号')"
                      />
                    </div>
                  </template>
                </div>
          ...
          <li v-if="qrCodeUrl" class="tips">
            <div style="text-align: left">{{ $t('温馨提示：') }}</div>
            <div style="text-align: left">
              {{ $t('1.扫码后请及时付款，付款成功后后请点击完成付款金额将确保及时为您充值成功；') }}
              <br />
              {{ $t('2.充值成功后不要再次使用本次二维码充值，充值后我们无法为您增加相应额度;') }}
              <br />
              {{ $t('3.如充值后未到账额度请联系在线客服！') }}
            </div>
          </li>
      ...
      >
        ...
        <div class="pop-body">
          ...
          <div class="ui-buttons">
            <van-button type="primary" @click="goPayment">
              <span style="font-weight: bold;">{{ $t('去支付') }}</span>
            </van-button>
            <van-button type="cancel" @click="popUpExplain = false">
              <span style="color: #ccc;font-weight: bold;">{{ $t('取消') }}</span>
            </van-button>
          </div>
        </div>
      </van-popup>
        ...
        <div class="pop-body iconImage">
          ...
          <div class="ui-buttons">
            <van-button type="primary" @click="isPayment(isExplain)">
              <span style="font-weight: bold;">{{ $t('已完成支付') }}</span>
            </van-button>
            <van-button type="cancel" @click="$openKefu">
              <span style="color: #ccc;font-weight: bold;">{{ $t('支付遇到问题') }}</span>
            </van-button>
          </div>
        </div>
      </van-popup>
        ...
        <div class="pop-body payStatus">
          ...
          <div class="btn">
            <div class="aagames-button-box fail-button">
              ...
              <button class="aagames-button" type="button" @click="paymentFail = false">{{ $t('确认') }}</button>
            </div>
      ...
      >
        ...
        <div class="pop-body payStatus">
          ...
          <div class="btn">
            <div class="aagames-button-box">
              <button class="aagames-button" type="button" @click="handleSuccess">{{ $t('确认') }}</button>
            </div>
          </div>
        </div>
      </van-popup>
...
import { paytype, order, checkorder, depositinfo, allbanklist, banklist, getprotocol, getticker } from "@/api/agent"
  ...
  watch: {
    active(val) {
      ...
      const data = {
        ...
        pay_method_id: String(this.payMethodId),
  ...
  methods: {
    ...
    getPaytypes() {
      ...
      paytype({
        ...
        console.log(res.data, this.$t('获取支付类型'))
        if (res.data.code === 0) {
    ...
    getDepositInfo() {
      ...
      depositinfo(data).then((res) => {
        ...
        /* console.log(this.info, 'depositInfo') */
    ...
    getMerchantBank() {
      let that = this
      merchantbank({
        pay_method_id: that.payMethodId,
      }).then((res) => {
