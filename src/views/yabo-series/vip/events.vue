<template>
  <div class="vip-page">
    <div
      :class="
        $projectname === '10042' ? 'events more-padding-bottom' : 'events'
      "
    >
      <template
        v-if="
          ...
          >
            <template #title>
              <van-image
                ...
                :src="
                  $imgs[
                    'vip/newLevel/vip_details_icon' + (index + 1) + '_normal@2x'
                  ]
                "
              />
              <van-image
                ...
                :src="
                  $imgs[
                    'vip/newLevel/vip_details_icon' + (index + 1) + '_Select@2x'
                  ]
                "
              />
        ...
        >
          ...
          <template v-if="activeNum === 2">
            <div class="main-block">
              <p class="block-tit">
                ...
                ><i>{{$t('VIP等级越高，每月红包金额越多')}}</i>
              </p>
              <p class="block-tit-sub">{{$t('特权简介')}}</p>
              <p class="block-desc">
                会员达到指定VIP等级，即可每月领取每月红包（月初自动发放），
                VIP等级越高，每月发放的红包金额越多！
              </p>
              <table class="venueStyle__monthlyTable">
                <colgroup>
          ...
          <template v-if="activeNum === 3">
            <div class="main-block">
              ...
              <p class="block-tit-sub">{{$t('特权简介')}}</p>
              <p class="block-desc">
                达到一定VIP等级的时候，平台将会在您生日的时候赠送大额的生
          ...
          <template v-if="activeNum === 4">
            <div class="main-block">
              ...
              <p class="block-tit-sub">{{$t('特权简介')}}</p>
              <p class="block-desc">
                会员晋级的时候，自动获得晋级礼金，VIP等级越高，晋级的时候
        ...
        <template v-else>
          <van-sticky :container="vip - page" v-if="!$route.query.source">
            <div class="vip-tab-head">
              ...
              <span :class="active === 0 ? 'on' : ''" @click="tabFn(0)"
                >{{$t('VIP特权')}}</span
              >
              <span :class="active === 1 ? 'on' : ''" @click="tabFn(1)"
                >{{$t('VIP详情')}}</span
              >
            </div>
          </van-sticky>
        </template>
        <h2 class="events-title" v-if="$projectname == '10042'">
          <i></i><span>{{$t('VIP尊享福利')}}</span>
        </h2>
        <h2 class="events-title" v-else><i></i><span>{{$t('VIP返水与比例')}}</span></h2>
        <ul class="newfs-wrap">
          <li class="newfs-wrap-tit">
            <div>{{$t('VIP等级')}}</div>
            <div v-for="(item, index) in 11">
              <img
      ...
      <template v-if="$projectname === '10025'">
        ...
        <div class="hlEvents">
          <div class="hlEvents-main-t">
            ...
            <div class="swiper-container swiper-rank">
              <div class="swiper-wrapper">
                <div class="swiper-slide" v-for="(item, index) in 11">
                  <img
                    :src="
                      $imgs['vip/newLevel/grade_normal' + (index + 1) + '@2x']
                    "
          ...
          />
          <p class="hlCopyright">
            - 版权所有 ©2019-2020 {{ $t($config[$projectname].project) }} 保留所有权
            -
          </p>
        </div>
      </template>
      <img
        v-if="
          ($projectname === '10042' || $projectname === '10043') &&
          showScrollTop
        "
        @click="scrollToTOP"
        class="back-top"
        :src="$imgs['vip/back-top@2x']"
        alt=""
      />
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    scrollToTOP() {
      let scrollY = document.getElementsByClassName("vip-page")[0].offsetTop;
    ...
    getLevelInfo() {
      ...
      memberlevellist().then((res) => {
          ...
          this.rules = [
        {
          title: this.$t('晋级标准'),
          con:
            this.$t('会员的累计投注额达到相应级别的要求，即可晋级相应VIP等级，数据统计时间从账户注册之后立即开始；'),
        },
        {
          title: this.$t('保级标准'),
          con:
            this.$t(`会员在“VIP等级变更”后的（${this.levelData[this.userInfo.level].deposit_day}）天内需完成相应等级的有效投注额和存款要求方可继续保级(${this.levelData[this.userInfo.level].deposit_day})天；`),
        },
        {
          title: this.$t('降级标准'),
          con:
            this.$t(`如果会员在（${this.levelData[this.userInfo.level].deposit_day}）天内没有完成相应的保级要求流水，系统会自动降级一个等级，福利也会随之调整至降级后的等级; `),
        },
        {
          title: this.$t('晋级礼金'),
          con:
            this.$t('会员晋级的时候，自动获得晋级礼金，VIP等级越高，晋级的时候获得的VIP晋级礼金也越多！每个玩家每个VIP等级只能获得1次晋级礼金。VIP等级降级后，再次晋级，不能重复获得晋级礼金。'),
        },
        {
          title: this.$t('生日礼金'),
          con:
            this.$t('达到一定VIP等级的时候，平台将会在您生日的时候赠送大额的生日礼金，一定要记得设置出生日哦！ 特别说明:会员在注册后90天内过生日，今年将不能领取生日礼金。另注册后出生日期大于90天的会员会按照您填写的出生日期自动派发生日彩金。'),
        },
        {
          title: this.$t('会员日红包'),
          con:
            this.$t('会员可在每个自然月的18号找客服领取每月红包，每月红包金额以当月18号当天VIP等级为准（每月会员日红包礼金3倍流水即可取款）；'),
        },
        {
          title: this.$t('佳节礼品'),
          con:
            this.$t('达到一定的VIP等级时您可以在中国法定节假日联系在线客服小姐姐，领取对应等级礼品哦！'),
        },
        {
          title: this.$t('客户经理'),
        ...
        },
        {
          title: this.$t('游戏通道'),
          con:
            this.$t(`您个人专属的域名和网络线路，更快更稳定，杜绝风险隐患；${this.$config[this.$projectname].project}有权对该活动进行修改、暂停、取消等，无需特别通知，并保留对该活动的最终解释权。`),
        },
      ]
        }
      });
