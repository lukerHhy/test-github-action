        ...
        <div class="aagames-form steps1">
          <div class="input username">
            ...
            <input
              type="text"
              v-model="username"
              :placeholder="`${$t('请输入您的')}${$t($config[$projectname].project)}${$t('账号')}`"
            />
          </div>
        </div>
        <div class="aagames-button-box step-one">
          <van-button
            :class="['aagames-button', { dark: !username }]"
            type="button"
            @click="checkUserInfo"
          >
{{$t('下一步')}}
      ...
      <div v-if="step === 10" class="no-binding">
        <div class="msg">
          由于您未绑定手机号、邮箱、未设置密保问题，无法验证修改密码!
        </div>
        <div class="aagames-button-box">
          <van-button class="aagames-button dark" type="button" @click="step = 1">
{{$t('绑定资料')}}
          </van-button>
          <div class="px30"></div>
          <van-button class="aagames-button dark" type="button" @click="$openKefu">
{{$t('在线客服解决')}}
          </van-button>
        </div>
      </div>
      <div class="retrievePassword" v-else-if="step === 2">
        <template v-if="selectWay">
          ...
          <div class="aagames-form verification">
            ...
            <div class="input" @click="show = true">
              ...
              <input
                type="text"
                disabled
                v-model="selectWay.text"
                 :placeholder="$t('选择验证方式')"
              />
            </div>
          </div>
          <!--<van-cell  :title="$t('请选择验证方式')" @click="show = true" is-link v-model="selectWay.text" />-->
          <div
            class="aagames-form verification"
            v-show="selectWay.type === 'mobile'"
          >
            <p>
              <span>{{$t('已绑定手机')}}</span>
              {{ userSafeinfo.mobile }}
          ...
          <div
            class="aagames-form verification"
            v-show="selectWay.type === 'question'"
          >
            <p>
              <span>{{$t('密保问题')}}</span>
              {{ question.name }}
          ...
          <div class="aagames-button-box">
            <van-button
              :disabled="!code && !answer"
              :class="['aagames-button', { dark: !code && !answer }]"
              type="button"
              @click="forgetFn"
            >
{{$t('下一步')}}
      ...
      <div class="reset" v-else-if="step === 3">
        <div class="newPass aagames-form">
          ...
          <div class="input">
            ...
            <input
              type="password"
              v-model="password"
              :placeholder="$t('8-12位字符，由数字、字母、符号组成')"
            />
          </div>
        </div>
        ...
        <div class="aagames-button-box">
          <van-button
            type="button"
            @loading="loading"
            :class="['aagames-button', { dark: !password || !repassword }]"
            @click="submit"
          >
{{$t('完成')}}
          </van-button>
