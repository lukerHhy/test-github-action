<template>
  <div class="container quota">
    <lheader
      ...
      path="111"
      :turn.sync="isOpen"
       :help="walletMoney.open_mode == 1"
    ></lheader>
    <div id="apply">
      <div class="main">
        <div class="accout">
          <div class="automatic" v-show="walletMoney.open_mode == 1">
            <span>{{$t('自动免转钱包')}}</span>
            <van-switch
              v-model="openMoney"
            ...
            >
              <p>
                {{ item.game_platform_name
                  ...
                  v-show="
                    list.to_platform_name !== '主账户' &&
                      activeIndex == index &&
                      !openMoney
                  "
                  >({{ list.to_platform_name }})</span
                >
              </p>
              <p>{{ (item.money || 0) | currency('') }}</p>
              <img
                :src="$imgs['memberCenter/normal']"
                alt
                v-if="item.hot == 1"
              />
            </li>
    ...
    <!-- 游戏平台列表 -->
    <van-popup class="custom" v-model="gameShow" position="bottom">
      <div class="head">
      ...
      <div class="list">
        <ul>
          <li v-for="(item, index) in chooseList" :key="index">
            ...
            >
              <p @click="choosePlat(item)" class="name on">
                {{ item.game_platform_name }}
              </p>
              <p @click="choosePlat(item)" class="money on">{{ item.money }}</p>
            </template>
            <template v-else>
              <p @click="choosePlat(item)" class="name">
                {{ item.game_platform_name }}
              </p>
            ...
            <img
              @click="shareMoney(item.money_code)"
              v-if="item.isShare && shareWallet[item.money_code].length > 1"
              class="shareImg"
              :src="$imgs['memberCenter/share']"
              alt=""
            />
          </li>
        </ul>
      ...
      <div class="choose">
        <div v-for="(item, index) in walletList" :key="index" class="father">
          ...
          >
            ...
            >
              {{ val.game_platform_name }}({{ val.money }})
            </p>
            <p @click="chooseSon(val)" class="name" v-else>
              {{ val.game_platform_name }}({{ val.money }})
            </p>
...
export default {
          ...
          data.forEach((a) => {
            if (a.platform_list) {
              a.platform_list.forEach((b) => {
                if (
                  b.money_code == item &&
                  this.shareWallet[item].indexOf(b.game_platform_name) == -1
                ) {
  ...
  watch: {
    ...
    walletMoney: {
      handler(val) {
        this.list.from_money = (
          val[this.list.from_money_code] * 1 || 0
        ).toFixed(2)
      },
      deep: true,
    },
