              ...
              <ul v-show="withdrawList.length !== 0">
                ...
                >
                  <li v-for="(item,index) in withdrawList" :key="index">
                    ...
                    <div class="list_content">
                      <div class="header">
                        <span>{{$t('尾号')}}{{ item.bank_card_no | intercept }}</span>
                        <span>-{{ item.money }}</span>
                      </div>
                      <div class="status">
                        <span>{{ item.created_at | filterLevel }} | {{ item.trade_no }}</span>
...
{{$t('转')}}
    ...
    <van-popup v-model="selectDisplay" :inner="true" :width="drawerWidth" :mask="false" close-icon-position="top-right" get-container="#app" class="close-top custom">
      ...
      <div class="list">
        ...
        <div class="select-bar" v-if="(active==0||active==1|| active==4 ||active==5)">{{$t('选择状态')}}</div>
