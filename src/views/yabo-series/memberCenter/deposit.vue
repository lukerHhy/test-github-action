        ...
        <ul class="deposit-main">
          ...
          <li>
            ...
            <!-- <template v-if="payType == 11 || payType == 15 || payType == 16">
              <label>{{$t('协议')}}</label>
              <div class="protocol">
                <span @click="handleProtocol(item.protocol)" :class="{'active':chosePtotocol == item.protocol}" v-for="(item,index) in protocolList" :key="index">{{item.title}}</span>
              </div>
            </template>-->
          </li>
          <!-- 快捷金额 -->
          <li class="quick-select">
            <div class="board">
              <div v-if="info.is_quick_money && moneyList.length > 0" class="chips">
                ...
                >
                  <label
                    v-if="getChipClassName(item) === 'chip1'"
                    :style="{ backgroundImage: $bImgs['chouma/chouma1'] }"
                    :class="{ disabledLabel: item * 1 < thirdPayMsg.money * 1 }"
                    @click="selectAmount(item, index)"
                    :disabled="item * 1 < thirdPayMsg.money * 1"
                  >{{ item }}</label>
                  <label
                    v-if="getChipClassName(item) === 'chip2'"
                    :style="{ backgroundImage: $bImgs['chouma/chouma2'] }"
                    :class="{ disabledLabel: item * 1 < thirdPayMsg.money * 1 }"
                    @click="selectAmount(item, index)"
                    :disabled="item * 1 < thirdPayMsg.money * 1"
                  >{{ item }}</label>
                  <label
                    v-if="getChipClassName(item) === 'chip3'"
                    :style="{ backgroundImage: $bImgs['chouma/chouma3'] }"
                    :class="{ disabledLabel: item * 1 < thirdPayMsg.money * 1 }"
                    @click="selectAmount(item, index)"
                    :disabled="item * 1 < thirdPayMsg.money * 1"
                  >{{ item }}</label>
                  <label
                    v-if="getChipClassName(item) === 'chip4'"
                    :style="{ backgroundImage: $bImgs['chouma/chouma4'] }"
                    :class="{ disabledLabel: item * 1 < thirdPayMsg.money * 1 }"
                    @click="selectAmount(item, index)"
                    :disabled="item * 1 < thirdPayMsg.money * 1"
                  >{{ item }}</label>
                  <label
                    v-if="getChipClassName(item) === 'chip5'"
                    :style="{ backgroundImage: $bImgs['chouma/chouma5'] }"
                    :class="{ disabledLabel: item * 1 < thirdPayMsg.money * 1 }"
                    @click="selectAmount(item, index)"
                    :disabled="item * 1 < thirdPayMsg.money * 1"
                  >{{ item }}</label>
              ...
              <div class="list-cells">
                ...
                <template v-if="payType == 11 || payType == 15 || payType == 16">
                  <div class="cell-protocol">
                    ...
                    <div class="protocol">
                      <span
                        @click="handleProtocol(item.protocol)"
                        :class="{ 'active': chosePtotocol == item.protocol }"
                        v-for="(item,index) in protocolList"
                        v-show="displayProtocol.includes(String(item.type))"
                        :key="index"
                      >{{ item.title }}</span>
                    </div>
                  </div>
                ...
                <div class="cell" v-if="payment && (payment.id === 12 || payment.id === 13)">
                  <template v-if="qrCodeUrl">
                    <div class="cell-head">{{ payment.id === 12 ? $t('微信') : $t('支付宝') }}昵称</div>
                    <div class="cell-body">
                      <input
                        type="text"
                        v-if="payment.id === 12"
          ...
          <li v-if="qrCodeUrl" class="tips">
            <span
              class="tips-html"
              v-html="$t('温馨提示：1.扫码后请及时付款，付款成功后后请点击完成付款金额将确保及时为您充值成功；2.充值成功后不要再次使用本次二维码充值，充值后我们无法为您增加相应额度3.如充值后未到账额度请联系在线客服！')"
            ></span>
          </li>
          <li v-else-if="payType == 11 || payType == 15 || payType == 16" class="tips ticker">
            ...
            <span>
              {{ $t('预计支付') }}
              <i>{{ (ticker ? form.money / ticker : 0).toFixed(2) }}</i>
              {{ channel }}
            </span>
          </li>
        ...
        <div class="list-cells" v-show="paymentsChild.length == 0">
          ...
          >
              ...
              <van-image
                ...
                :src="
                  item.id !== 8 ?
                    ...
                    require(`@/assets/common/img/ucenter/dc@2x.png`)
                "
              />
            </div>
            <div class="cell-body">
              <div class="cell-flex">{{ item.alias || item.name }}</div>
            </div>
            <div class="cell-foot" v-show="(payment && payment.id === item.id) || item.child">
              ...
              <van-icon v-show="item.child" name="arrow" color="#333" />
            </div>
          </div>
        </div>
        <div class="list-cells" v-show="paymentsChild.length > 0">
      ...
      >
        ...
        <div class="pop-body">
          ...
          <div class="ui-buttons aagames-button-box">
            <!--            <van-button type="cancel" @click="popUpExplain = false">{{$t('取消')}}</van-button>-->
            ...
            >支付遇到问题？</button>
            <button
              class="aagames-button van-button--primary"
              type="button"
              @click="isPayment(popUpExplain)"
            >{{ $t('已完成支付') }}</button>
          </div>
        </div>
      </van-popup>
...
export default {
    ...
    'form.money': {
      handler(newValue, oldValue) {
        if (newValue && this.form.activityId) {
          this.flowText = `流水要求：流水倍数${this.flow
  ...
  methods: {
    ...
    async getProtocolList() {
      let res = await getprotocol({ pay_method_id: this.payment.pay_method_id })
      /* this.protocolList = res.data.data */
      this.protocolList = res.data.data.map((item) => ({
        ...item,
        type: item.title === 'ERC20' ? '1' : '2',
      }));
      /* console.log('res', this.protocolList) */
    ...
    async handleTicker(val) {
      let res = await getticker({ digital_mode_type: val, pay_method_id: this.payment.pay_method_id })
      this.ticker = res.data.data.upCoin
      console.log('ticker', this.ticker, this.payType, res)
    },
    initForm() {
      this.form = {
        money: '',
    ...
    getDepositInfo() {
      ...
      this.payMethodId && depositinfo(data).then((res) => {
        ...
        if (res.data.data && res.data.data.length > 0) {
          ...
          this.banksForPayOnline.forEach((item) => {
            item.name = `<div style='position: relative;'>
              <div style="display: ${item.isDisabled ? 'block' : 'none'
              };height: 44px;
              position: absolute;
              width: 100%;
              padding: 0 16px;
              ...
              <img style="height: 26px;vertical-align: middle;margin-right:10px;" src="${this.bankCodeArr.indexOf(item.icon_code) == -1
    ...
    getActivities() {
      ...
      if (this.$route.params.table) {
        ...
        this.activities.push({
          id: '',
          flow: 1,
          name: `大转盘-存款${table.recharge_money}送${table.gift_money}兑换券`,
          table: table,
        })
      } else {
    ...
    beforeSubmit() {
      openUrl(this.submit, this.payType)
    },
    submit(fn) {
      const {
        form,
        ...
        .then((res) => {
          if (res.data.code === 0) {
            ...
            if (
              ...
              return Promise.resolve(this.order.pay_url)
            }
          } else {
            this.$toast(res.data.msg)
          }
...
.van-popup.custom.pay-res {
  ...
  .pop-body.payStatus {
    .aagames-button-box.fail-button,
    .success-button {
      margin-top: 40px;
      justify-content: space-between;
      button {
