    ...
    <div class="main">
      <div id="register">
        <div class="register" align="center">
          ...
          <p>{{$t('恭喜您已成为')}}{{$t($config[$projectname].project)}}{{$t('尊贵会员')}}</p>
          <!--<div>{{$t('您的账号密码')}}</div>-->
        </div>
        <div class="accountInfo" id="capture">
            ...
            <div class="aagames-form">
              <div class="input">
                ...
                <input
                  ...
                  :placeholder="'输入a开头6-12位的英文字母数字'"
                  type="text"
                  v-model="username"
                />
                <span v-else>{{ username }}</span>
                <template v-if="registerType === 'quick' && usernameCanEdit">
                  <a v-if="!accountOnEditingU" @click="accountOnEditingU = true">{{$t('修改')}}</a>
                </template>
              </div>
            </div>
            <div class="aagames-form">
              <div class="input">
                ...
                <template v-if="passwordCanEdit">
                  <a v-if="!accountOnEditingP" @click="accountOnEditingP = true">{{$t('修改')}}</a>
                </template>
              </div>
            </div>
          </div>
          <template v-if="accountOnEditingP || accountOnEditingU">
            <div class="aagames-button-box buttons">
              <van-button class="aagames-button line" type="default" @click="hanlerCancel">{{$t('取消修改')}}</van-button>
              <div class="px30"></div>
              <van-button class="aagames-button line" type="primary" @click="updateAccount">{{$t('提交保存')}}</van-button>
            </div>
          </template>
          <template v-else>
            <div class="aagames-button-box buttons">
              ...
              <button class="aagames-button line" type="button" style="font-weight:600;" @click="toPerson">{{$t('完善个人资料')}}</button>
            </div>
          </template>
          <!-- <div>
                  <span>{{$t('您的账户密码')}}</span>
  ...
  created() {
    ...
    if (!username) {
      if(['10042'].includes(process.env.VUE_APP_PROJECT_NAME)){
        return this.$store.dispatch("global/setLoginPopShow", true)
      }else{
