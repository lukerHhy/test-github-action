<template>
  <div
    class="member-page"
    :style="{ backgroundImage: $bImgs['memberCenter/bg@2x'] }"
  >
    <van-nav-bar
      class="m-header"
       :title="$t('我的')"
    ...
    >
      ...
      >
        <img
          @click="onClickRight"
          class="msg-img"
          :src="$imgs['memberCenter/person_icon1']"
          alt
        />
        <img
          @click="onClickLeft"
          class="more-img"
          slot="right"
          :src="$imgs['memberCenter/person_icon2']"
        />
      </div>
    </van-nav-bar>
    ...
    <div class="m-body">
      ...
      <div class="user-info">
        <div class="person-info">
          ...
          <img
            @click="listPath('personalData')"
            :src="$imgs['agent/agent_icon7']"
            alt="login-out"
          />
          <div class="right">
            <div class="top">
              ...
              <template v-else>
                <span
                  ...
                  @click="
                    () => {
                      !isLogin && this.$router.push({ path: '/login' })
                    }
                  "
                  >{{$t('登录')}}</span
                >
                <span
                  ...
                  @click="
                    () => {
                      !isLogin && this.$router.push({ path: '/register' })
                    }
                  "
                  >{{$t('注册')}}</span
                >
              ...
              <span
                ...
                v-if="isLogin"
                @click="govip"
                :style="{ backgroundImage: $bImgs['vip/level-small/vipbg@2x'] }"
              >
            ...
            <div
              class="bottom bottom-progress"
              v-if="
                isLogin & ($projectname === '10023') || $projectname === '10033'
              "
            >
              <div>
                <span
                  class="bottom-progress-vip"
                  v-if="$projectname === '10033'"
                  >VIP{{ userInfo.level }}</span
                >
                <van-progress
                  ...
                  track-color="#3E64D8"
                />
                <span class="bottom-progress-vip"
                  >VIP{{ userInfo.level + 1 }}</span
                >
              </div>
              升级还需<span>{{
                basicInfo.need_valid_bet - basicInfo.all_valid_bet
              }}</span>
              元 流水
            </div>
          </div>
...
{{$t('理财小金库')}}
                ...
                <span>{{
                  ((wallet && wallet.financing_money) || 0) | currency('')
                }}</span>
              </div>
              <div
                class="info-area"
                @click="listPath('quotaTransfer')"
...
{{$t('游戏额度')}}
                ...
                <span>{{
                  ((wallet && wallet.all_platform_money) || 0) | currency('')
                }}</span>
              </div>
            </div>
            <!--<div class="operating">-->
        ...
        <template v-if="!$config[$projectname].hasXcoin">
          <ul class="quick-link">
            <li @click="listPath('deposit')">
              <img :src="$imgs['agent/agent_icon13']" alt="" /><span>{{$t('充值')}}</span>
            </li>
            <li @click="listPath('washcode')">
              <img :src="$imgs['agent/agent_icon14']" alt="" /><span>{{$t('洗码')}}</span>
            </li>
            <li @click="handleWithdraw">
              <img :src="$imgs['agent/agent_icon15']" alt="" /><span>{{$t('取款')}}</span>
            </li>
            <li @click="govip">
              <img :src="$imgs['agent/agent_icon16']" alt="" /><span>VIP</span>
            </li>
          </ul>
        </template>
        <template v-else>
          <ul class="quick-link">
            <li @click="listPath('deposit')">
              <img :src="$imgs['home/home_iocn1@2x']" alt="" /><span>{{$t('充值')}}</span>
            </li>
            <li @click="showXcoin = true">
              <img :src="$imgs['memberCenter/xima@2x']" alt="" /><span
                >{{$t('X币兑换')}}</span
              >
            </li>
            <li @click="handleWithdraw">
              <img :src="$imgs['home/home_iocn3@2x']" alt="" /><span>{{$t('取款')}}</span>
            </li>
            <li @click="govip">
              <img :src="$imgs['home/home_iocn4@2x']" alt="" /><span>VIP</span>
            </li>
          </ul>
        </template>
      </div>
      <div class="latest">
        <h2
          @click="
            () => {
              withdrawListsMy.length || depositListsMy.length
                ? $router.push({ name: 'businessRecord', query: { type: 5 } })
                : $router.push({ name: 'WithdrawRank' })
            }
          "
        >
          <span>{{$t('最新动态')}}</span>
...
{{$t('出款成功')}}
        ...
        <template v-if="!withdrawLists.length">
          <template
            v-if="withdrawListsMy.length > 0 && depositListsMy.length == 0"
          >
            <div
              class="newest-info-detail"
            ...
            >
              <div class="newest-info-first">
                <div class="newest-name">
                  {{$t('取款-')}}{{ item.username }}
                  <span class="push-money" @click="$toast('系统已优先为您处理')"
                    >{{$t('催一下')}}</span
                  >
                  <!-- <img class="levelImg" :src="require(`@assets/img3_0/vip/level-small/LV${ item.level}@2x.png`)" alt=""> -->
                </div>
          ...
          <template
            v-if="depositListsMy.length > 0 && withdrawListsMy.length == 0"
          >
            <div
              class="newest-info-detail"
        ...
        <van-row align="center" :gutter="2">
          ...
          >
            <i
              :class="'list' + (index + 1)"
              :style="{
                backgroundImage: $bImgs['memberCenter/icon' + (index + 1) + ''],
              }"
            ></i>
    ...
    <div class="popup levelUp" v-show="isPop && userInfo && userInfo.level">
      ...
      <div
        ...
        :style="{ backgroundImage: $bImgs['memberCenter/levelUp/sj_bg@2x'] }"
      >
        <!--<img class="levelImg" :src="require(`@assets/img3_0/vip/level/LV${ userInfo.level}@2x.png`)" alt="">-->
        <img
          class="levelImg"
          :src="$imgs['vip/level-small/LV' + userInfo.level + '@2x']"
          alt=""
        />
        <div class="upgrade">
          <b>{{$t('恭喜您')}}</b>
          ...
          <b class="level-text"
            >{{$t('升级到')}}{{
              (userInfo && userInfo.level) | filterLevel(levelName)
            }}{{$t('会员')}}</b
          >
        </div>
        <div class="detailed">
          <div v-for="(item, index) in profitArr" :key="index">
            ...
            <img
              :src="$imgs['vip/level-small/LV' + item.thumb + '@2x']"
              alt=""
            />
...
import { vipBasicInfo } from '@/api/vip'
...
export default {
  ...
  created() {
    ...
    Promise.all([this.getWithdrawListsMy(1), this.getWithdrawListsMy(2)]).then(
      (val) => {
        ...
        if (val[0].length || val[1].length) {
          this.timer = setInterval(() => {
            Promise.all([
              this.getWithdrawListsMy(1),
              this.getWithdrawListsMy(2),
            ]).then((val) => {
              if (!val[0].length && !val[1].length) {
  ...
  methods: {
    ...
    handleWithdraw() {
      ...
      if (!bankList || !bankList.length) {
        ...
        this.$dialog
          .confirm({
            message: `${this.$t('您未绑定银行卡，我们无法为您打款到账户')}<br>${this.$t('遇到问题')} <span style="color:${this.$colorjs.baseColor};">联系专属客服<span>`,
