    ...
    <div class="main">
      ...
      <div class="retrievePassword" v-else-if="step === 2">
        <template v-if="selectWay">
          ...
          <div class="aagames-button-box">
            <button :class="['aagames-button',{'dark':!code && !answer }]" type="button" @click="forgetFn">{{$t('下一步')}}</button>
          </div>
        </template>
      </div>
      ...
      <div class="reset" v-else-if="step === 3">
        ...
        <div v-show="routeType != 'withdraw'" class="aagames-button-box">
          <button type="button"  :class="['aagames-button',{'dark':routeType == 'withdraw'?(!setPassData.password || !setPassData.repassword):(!setAnswer.answer || !setAnswer.re_answer || !problem)}]" @click="submit">{{$t('完成')}}</button>
        </div>
      </div>
      <!-- 校验下拉 -->
