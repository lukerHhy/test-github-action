  ...
  <div class="container forgerPassword">
    ...
    <div class="main">
      ...
      <div class="retrievePassword" v-else-if="step === 2">
        <template v-if="selectWay">
          ...
          <div class="aagames-form verification" v-show="selectWay.type === 'mobile'">
            ...
            <div class="code">
              <div class="input">
                <img class="img" style="margin-right: 6px;" :src="require('@assets/img3_0/otherIcon/mbwt@2x.png')" alt />
          ...
          <div class="aagames-button-box">
            <van-button :class="['aagames-button',{'dark':!code && !answer }]" type="button" :disabled="!code && !answer" @click="forgetFn">{{$t('下一步')}}</van-button>
          </div>
        </template>
      </div>
      ...
      <div class="reset" v-else-if="step === 3">
        ...
        <div class="aagames-button-box">
          <van-button @loading="loading" type="button"  :class="['aagames-button',{'dark':!password || !repassword}]" :disabled="!password && !repassword" @click="submit">{{$t('完成')}}</van-button>
        </div>
      </div>
      <!-- 校验下拉 -->
      <van-popup
