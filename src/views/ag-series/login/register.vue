    ...
    <div class="main" id="register">
      <div class="register">
        <div class="top-bar">
          ...
          <a href="javascript:;" @click="go('/login')">{{$t('登录')}}</a>
        </div>
        ...
        <div class="fill">
          ...
          <template v-if="active === 2">
            <div class="aagames-form" style="margin-bottom: 0">
              <div class="input">
                <img class="img phone" :src="$imgs['login/mobile@2x']" alt />
...
<!--              <i v-else class="checkOff"></i>-->
              <van-checkbox v-model="isAgree" :checked-color="$colorjs.baseColor">
{{$t('同意并接受')}}
              </van-checkbox>
...
export default {
  ...
  methods: {
    ...
    showProtocol() {
      ...
      this.$dialog
          .alert({
            title: `${this.$config[this.$projectname].secName}${$t('游戏条款')}`,
            message: html,
            messageAlign: 'left'
          })
          .then(() => {
