...
<!--            <van-icon v-if="mwallte.hasOwnProperty('day_rate')" @click="showMsg" name="question" />-->
            <van-image v-if="mwallte.hasOwnProperty('day_rate')" @click="showMsg" class="help-icon" :src="$imgs['memberCenter/help@2x']"></van-image>
          </div>
          <!-- 收益统计 -->
          <div class="income">
            <div class="total-amount">
...
<!--          <button type="primary" class="btn" @click="sure">{{$t('确认')}}</button>-->
          <van-button type="primary" class="btn" @click="sure">{{$t('确认')}}</van-button>
           <div class="text">{{$t('取款流水验证有')}}</div>
        </div>
      </div>
    </van-popup>
...
${this.$t('使用理财小金库可让您额外获得高额利息，收益率将按照转入金额*不同等级计算。发放时间为每天下午4点准时发放到您的理财钱包。')}`,
  ...
  mounted() {
    ...
    document.querySelector('.container').onscroll = function (){
      ...
      scrollTop ? that.isTop = true : that.isTop = false
    }
  }
};
</script>
